...
<script id="mmw-order-catalog-script">(()=>{const KEY='MMWCompanyProductsCart';const CATALOG=${JSON.stringify([...ORDER_CATALOG.packages,...ORDER_CATALOG.services])};const read=()=>{try{const v=JSON.parse(localStorage.getItem(KEY)||'[]');return Array.isArray(v)?v.filter(x=>x&&typeof x.id==='string'&&Number.isFinite(Number(x.qty))&&Number(x.qty)>0).map(x=>({id:x.id,qty:Math.max(1,Math.floor(Number(x.qty)))})):[]}catch(e){return[]}};const write=cart=>{localStorage.setItem(KEY,JSON.stringify(cart));window.dispatchEvent(new CustomEvent('mmw-cart-updated'));};const get=id=>CATALOG.find(x=>x.id===id);const render=()=>{const cart=read();document.querySelectorAll('[data-mmw-order-id]').forEach(b=>{const x=cart.find(i=>i.id===b.dataset.mmwOrderId);b.classList.toggle('added',!!x);b.textContent=x?'Добавлено ✓':'Добавить в корзину'});};document.addEventListener('click',e=>{const b=e.target.closest('[data-mmw-order-id]');if(!b)return;const id=b.dataset.mmwOrderId;if(!get(id))return;e.preventDefault();const cart=read();const x=cart.find(i=>i.id===id);if(x)x.qty=Math.max(1,Math.floor(Number(x.qty)))+1;else cart.push({id,qty:1});write(cart);render();});window.addEventListener('mmw-cart-updated',render);render()})();</script>`;
