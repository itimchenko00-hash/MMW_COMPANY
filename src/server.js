...
const BUILD='2026-09-02-mmw-company-routing-v3';

// Deployment invariant: never start a broken build without the canonical home page.
if(!fs.existsSync(MAIN_PATH)){
  console.error(`[MMW-FATAL] Missing canonical home page: ${MAIN_PATH}`);
  process.exit(1);
}
...
function sendHtml(file,res){
  fs.readFile(file,'utf8',(err,html)=>{
    ...
    let scripts='<script src="/public/mmw-language.js"></script>';
    const normalized=file.replace(/\\/g,'/');
    const isAladin=normalized.includes('/projects/ALADIN/');
    if(!isAladin)scripts+='<script src="/public/mmw-site-unifier.js"></script>';
    if(normalized.includes('/projects/NEXUS-WORK/'))scripts+='<script src="/public/nexus-work-product.js"></script><script src="/public/nexus-work-economics.js"></script>';
    if(normalized.includes('/projects/NEXUS-LOGISTICS/'))scripts+='<script src="/public/nexus-logistics-upgrade.js"></script><script src="/public/nexus-logistics-product-economics.js"></script>';
    if(normalized.includes('/projects/CARPATHIA/'))scripts+='<script src="/public/carpathia-upgrade.js"></script>';
    if(!html.includes('/public/mmw-language.js'))html=html.replace('</head>',scripts+'</head>');
...
// CANONICAL HOME: every historical/alternative home URL resolves here.
...
// Last-resort HTML fallback: prevents a missing home alias from ever producing "Cannot GET".
app.use((req,res,next)=>{
  if(req.method==='GET' && (req.path.endsWith('.html')||req.path==='/'))return res.redirect(308,'/');
  next();
});
